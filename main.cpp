...
double timeAddition(size_t n) {
    ...
    for (size_t i = 0; i != n; ++i) {
        const auto a = BigUInt::createRandomFromDecimalDigits(10000ul);
        const auto b = BigUInt::createRandomFromDecimalDigits(10000ul);
        const auto c = a + b;
    }
...
double timeDivision(size_t n) {
    Timer timer;
    for (size_t i = 0; i != n; ++i) {
        const auto a = BigUInt::createRandomFromDecimalDigits(15000ul);
        const auto b = BigUInt::createRandomFromDecimalDigits(10000ul);
        const auto c = a / b;
    }
    return timer.elapsed() / n;
